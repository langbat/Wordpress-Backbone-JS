...
function et_ajax_payment_process() {
    et_write_session('is_payment_zertifikat', 0);
    ...
    if ($price == 0)
        ...
        $plans = et_get_payment_plans_amount($price);
    ...
    et_write_session('job_form_data', $_POST);
...
function et_ajax_payment_zertifikat() {
    global $user_ID;
    $authorID = isset($_POST['authorID']) ? $_POST['authorID'] : $user_ID;
    $paymentType = isset($_POST['paymentType']) ? $_POST['paymentType'] : '';
    $price = isset($_POST['price']) ? $_POST['price'] : 0;
    $method = isset($_POST['method']) ? $_POST['method'] : '';
    $title = "Payment Zerfitikat";
    ...
    $company_location = et_get_user_field($user_ID, 'recent_job_location');
    $ship = array('street_address' => isset($company_location['full_location']) ? $company_location['full_location'] : __("No location", ET_DOMAIN));
    $plan = et_get_payment_zertifikat($order_data);
    $plan['ID'] = $authorID;
    // insert order into database
    $order = new ET_JobOrder($order_data, $ship);
    ...
    et_write_session('order_id', $order_data['ID']);
    et_write_session('job_id', $authorID);

    if ($method == "payment_zertifikat") {
        et_write_session('method', $method);
        et_write_session('is_payment_zertifikat', 1);
        if ($paymentType != 'paypal'){
            setUserZerfifikat($authorID, 0);
        }
    }
...
add_action('wp_ajax_et_payment_zertifikat', 'et_ajax_payment_zertifikat');

function et_ajax_payment_debit() {
    global $user_ID;
    ...
    $result .="\n";
    $result .=$payer . ";" . $price . ";" . $accountNumber . ";" . $bankNumber . ";" . $company_location['full_location'] . ";" . $order_date . "";
    $result .="\n";
    file_put_contents(getcwd() . "/wp-content/uploads/files/BeispielCSV.csv", $result);// . $filename
    //exit;
}

add_action('wp_ajax_et_payment_debit', 'et_ajax_payment_debit');
...
function je_update_current_user_order($company_id, $packageID, $order_id) {
    ...
    // return	update_option ('je_company_current_order' , $group );
...
function et_update_company_plans($company_id, $plan_id, $subtract = true) {
    ...
    if (empty($company_plans))
        $company_plans = array();

    // get the plan
    $plans = et_get_payment_plans();
    if (empty($plans[$plan_id]))
        return false; // if plan doesn't exist, return false
    if (isset($plans[$plan_id]['price']) && $plans[$plan_id]['price'] = 0)
        return 0; // if plan is free, doesn't count
...
// get quantity
    ...
    if ($quantity >= $plans[$plan_id]['quantity'])
        $quantity = 0;
...
function et_add_authorize_button($payment_gateways) {
    ...
    if (!isset($payment_gateways['authorize']))
        return;
    $authorize = $payment_gateways['authorize'];
    if (!isset($authorize['active']) || $authorize['active'] == -1)
        return;
    ?>
    <li class="clearfix">
        <div class="f-left">
...
function et_authorize_response($response
, $paymentType, $order) {
...
function et_setup_authorize() {
    ...
    <?php _e("Your Authorize API MD5 Hash ", ET_DOMAIN); ?>
                ...
                <span class="icon <?php
                      if ($authorize['x_MD5_hash'] == '')
                          echo 'color-error'
                          ?>" data-icon="<?php data_icon($authorize['x_MD5_hash']) ?>"></span>
            </div>
