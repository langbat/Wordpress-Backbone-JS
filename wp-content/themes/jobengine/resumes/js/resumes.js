...
JobEngine.Views.ResumeListView = Backbone.View.extend({
	...
	onAddResumes: function(resume, col,options ){
		...
		// if(typeof resume.get('method') != 'undefined' && resume.get('method') == 'reviewResume')
		// 	method = 1;
		// var listView = this.$el.find('ul');
		...
		// //var after_id = ;
		// var rid = this.convertTimeToID(resume.get('post_date'));
		// var lists = new  Array();
		// var filter = this;
		// var i = 0;
		// _.each(this.collection.models, function(resume, index, list){			
		// 	lists[i] = filter.convertTimeToID(resume.get('post_date'));
		// 	i++;
		// });
		// var pos = this.getPositionAppend(lists,rid);
		
		
		// if(this.collection.length > 1){			
		// 	if(pos == 1) // prepend item to top
		// 	 	listView.prepend(item.renderAprove(method).$el.hide().fadeIn());
		// 	else if(pos == -1) // prepend item to bottom
		// 		listView.append(item.renderAprove(method).$el.hide().fadeIn());
		// 	else // append item  affter element selected ;
		// 		listView.find('#' + pos).after(item.renderAprove(method).$el.hide().fadeIn());
		// }
		// else 
		// 	listView.html(item.render(method).$el.hide().fadeIn());
	},

	// getPositionAppend : function(args,rid){		
	// 	var pos  = args[0];	
...
// JobEngine.Views.EditedSkillItem = JobEngine.Views.EditedItem.extend({
// 	initialize: function(data){
// 		JobEngine.Views.EditedItem.prototype.initialize.call(data);
// 		this.data 		= data
// 		this.template 	= _.template($('#edit_skill_item').html());
// 	}
// });

// JobEngine.Views.EditedPositionItem = JobEngine.Views.EditedItem.extend({
// 	initialize: function(data){
// 		JobEngine.Views.EditedItem.prototype.initialize.call(data);
// 		this.template = _.template($('#edit_position_item').html());
// 	}
// });
...
JobEngine.Views.ExperienceView = Backbone.View.extend({
	...
	validate: function(){
		...
		if ( toDate < fromDate && !data.current && data.toMonth != '' && data.toYear != '' ){
			errors.push({ element: this.$el.find('select.fromMonth'), 'msg' : et_resume.date_range_invalid });
			errors.push({ element: this.$el.find('select.fromYear'), 'msg' :  et_resume.date_range_invalid });
			errors.push({ element: this.$el.find('select.toMonth'), 'msg' : et_resume.date_range_invalid });
			errors.push({ element: this.$el.find('select.toYear'), 'msg' : et_resume.date_range_invalid });
			//return this.errorResp('Begining date must after to finish date');
		}
