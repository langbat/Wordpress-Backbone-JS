...
class JE_Helper extends ET_Base{
	...
	static public function yearSelectBox( $name, $selected = false, $args = array()){
		...
		$to 	= date('Y');
		?>
		<select name="<?php echo $name ?>" class="<?php $args['class']?>" <?php if (!empty($args['id'])) echo 'id="' . $args . '"' ?> >
	...
	static public function yearSelectBoxTemplate( $name, $selected = false, $args = array()){
		...
		<select name="<?php echo $name ?>" class="<?php echo $args['class']?>" <?php if (!empty($args['id'])) echo 'id="' . $args . '"' ?> >
			...
			<?php for($i = $to; $i > $from ; $i--){ 
				...
				/* <option value="<?php echo $i ?>" <% if ( fromYear == 1 ) { %> selected="selected" <% } %> ><?php echo $i ?></option> */
				} else {
	...
	static public function jobPositionSelectTemplate($name, $selected = false, $args = array(), $field = 'term_id'){
		...
		//$data 	= get_terms('job_position', array('hide_empty' => false));
		?>
		<select name="<?php echo $name ?>" class="<?php echo $args['class']?>" <?php if (!empty($args['id'])) echo 'id="' . $args . '"' ?> >
			<option value=""><?php _e('Select your matches', ET_DOMAIN) ?></option>
			<?php //foreach ($data as $value) { ?>
				<?php self::job_cat_children_options($field, 'resume_category', $data); ?>
				<!-- <option value="<?php echo $value->$field ?>" <?php if ($value->term_id == $selected) echo 'selected="selected"' ?>><?php echo $value->name ?></option>  -->
			<?php //} ?>
		</select>
		<?php
	}

	private function job_cat_children_options($field, $tax, $cats = array(), $parent = false, $level = 0){
		...
		foreach ($cats as $cat) {
			if ( ($parent == false && !$cat->parent) || $parent == $cat->parent ){
				...
				self::job_cat_children_options($field, $tax, $cats, $cat->term_id, $level + 1);
			}
		} 
	}
