    ...
    function print_filter_list($query_obj, $args) {
        ...
        <ul class="job-filter filter-jobtype filter-joblist tax-filter"  data-tax='job_type'>
            ...
            foreach ($types as $type) {
                if ($hide_empty && $type->count <= 0)
                    continue;
                ?>
                <li>
                    <a data="<?php echo $type->slug ?>" href="<?php echo $this->get_term_link($type) ?>" class="color-<?php echo $colours[$type->term_id] ?> <?php if (($query_obj && $query_obj->term_id == $type->term_id) || in_array($type->slug, $queried_job_type)) echo 'active'; ?>">
...
/*
 * @author Ty Tran
 * Get field checkbox for Create Job
 */
add_action('wp_ajax_et_conditions_sync', 'et_ajax_load_conditions_post_job');
add_action('wp_ajax_nopriv_et_conditions_sync', 'et_ajax_load_conditions_post_job');
if (!function_exists('et_ajax_load_conditions_post_job')) {

    function et_ajax_load_conditions_post_job() {
        $args = $_REQUEST;
        $field_id = getConditionFiledId($args['job_types']);
        ...
        if (isset($job_id) && $job_id != '') {
            $types = get_post_meta($job_id, 'cfield-' . $field_id, true);
            if (!empty($types[$field_id]) && is_array($types)) {
                $types = $types[$field_id];
            }
        }
        if ($field_id) {
            $field = JEP_Field::get_field($field_id);
            $options = JEP_Field::get_options($field_id);
            echo '<script>$("#load_conditions_by_job_type").parent().show();</script>';
            echo '<div class="label">
                    <h6 class="">' . $field->name . '</h6>
                        ' . $field->desc . '
                </div>';
            echo '<div class=""><ul>';
            foreach ($options as $option) {
                $checked = '';
                if (!empty($types) && $types != 'null' && $types != null) {
                    $checked = in_array($option->ID, $types) ? 'checked' : '';
                }
                echo '<li><label><input type="checkbox" ' . $checked . ' name="cfield[' . $field_id . '][]" value="' . $option->ID . '">&nbsp; ' . $option->name . '</label></li>';
            }
            echo '</ul></div>';
            exit;
        } else {
            echo '<script>$("#load_conditions_by_job_type").parent().hide();</script>';
            exit;
        }
    }
    ...
    function et_ajax_job_handler() {
        ...
        if ($method == "edit_job") {
            ...
            foreach ($fields as $key => $field) {
                $args['cfield-' . $key . ''] = $field;
            }
            $args['cfield-1656'] = $args['cfield-1659'] = $args['cfield-975'] = $fields;

            //setup map
            if ($args['full_location'] == '' || $args['full_location'] == 'Anywhere' || $args['full_location'] == __('Anywhere', ET_DOMAIN)) {
                $args['full_location'] = __('Anywhere', ET_DOMAIN);
                $args['location'] = __('Anywhere', ET_DOMAIN);
                $args['location_lat'] = '';
                $args['location_lng'] = '';
            }
            ...
            et_write_session('job-content', $_REQUEST['content']);
        }

        $args['verbandverwaltung'] = '';
        $verbands = array();
        if ($method == "create" || $method == "edit_job") {
            for ($i = 1; $i < 2; $i++) {
                if ($args['verband_' . $i] != 'null')
                    $verbands = array_merge($verbands, array($args['verband_' . $i]));
            }
        }
        if (!empty($verbands)) {
            $verbands = array_unique($verbands);
            $args['verbandverwaltung'] = $verbands;
        }
        // validate here, later
...
function et_job_handler_creat_update_post($args, $method) {
    try {
        ...
        if (( current_user_can('edit_posts') && $args['author_id'] == $user_ID) || current_user_can('edit_others_posts') || !isset($args['id'])) {
            ...
            if (($args['apply_method'] == 'ishowtoapply' && $args['applicant_detail'] == '') || ($args['apply_method'] == 'isapplywithprofile' && $args['apply_email'] == '') || $args['title'] == "" || $args['content'] == "") {
            ...
            $cfield = array(
                'verbandverwaltung' => 'verbandverwaltung',
                'cfield-1036' => 'cfield-1036',
                'cfield-984' => 'cfield-984',
                'cfield-992' => 'cfield-992',
                'cfield-993' => 'cfield-993',
                'cfield-1096' => 'cfield-1096',
                'cfield-983' => 'cfield-983',
                'cfield-975' => 'cfield-975',
                'cfield-1659' => 'cfield-1659',
                'cfield-1656' => 'cfield-1656'
            );

            if ($method == "edit_job") {
                $key_map['job_types'] = 'job_type';
                $key_map['categories'] = 'category';
                $key_map['method'] = 'method';
            }
            ...
            if (isset($job_data['id']) && !empty($job_data['id']) && is_numeric($job_data['id'])) {
                ...
                if (current_user_can('edit_others_posts')) {
                    ...
                    if ($method == 'edit_job') {
                        if (!is_wp_error($job_id)) {
                            foreach ($args as $key => $val) {
                                if (isset($cfield[$key]) && !empty($cfield[$key])) {
                                    try {
                                        update_post_meta($job_id, $key, $val);
                                    } catch (Exception $e) {
                    ...
                    if ($user_ID == $job->post_author) {
                        ...
                        throw new Exception(__('You are not the job author', 401));
                    }
                }
            } else {
                ...
                //Update job cfield
                if (!is_wp_error($job_id)) {
                    if (isset($args['verbandverwaltung'])) {
                        try {
                            update_post_meta($job_id, 'verbandverwaltung', $args['verbandverwaltung']);
                        } catch (Exception $e) {
                            return new WP_Error($e->getCode(), $e->getMessage());
                        }
                    }
            ...
            if (!is_wp_error($job_id)) {
                ...
                if ($method == 'update' || $method == 'edit_job') {
                    je_edit_job_mail($job_id);
                ...
                if (isset($job_data['is_use_package']) && $job_data['is_use_package']) {
                    ...
                    //ngocanh: it redirect here before choose payment method
                ...
                if (isset($job_data) && !empty($job_data)) {
                    ...
                    if ($method == 'create' && isset($job_data['author_data']) && !empty($job_data['author_data'])) {
                        $data = $job_data['author_data'];
                        $author_data['user_url'] = $data['user_url'];
                        $author_data['display_name'] = $data['display_name'];
                    } elseif (isset($args) && !empty($args) && ($method == 'edit_job' || $method == 'update')) {
                        $author_data['user_url'] = $args['user_url'];
                        $author_data['display_name'] = $args['display_name'];
                    }
                    ...
                    if (current_user_can('edit_users') || current_user_can('administrator') || $job_data['post_author'] == $user_ID) {
                        ...
                        throw new Exception(__("You do not have permission to change this company's information!", 401));
                    }
                } else {
...
function et_prepare_job_query() {
    global $post, $et_global, $list_title;

    // refine meta query
    if (isset($_REQUEST['et_act']) && $_REQUEST['et_act'] == "filter_search")
        $request = $_REQUEST;
    ...
    if (isset($request['job_types']))
        $request['job_type'] = $request['job_types'];
    ...
    try {
        ...
        switch ($method) {
            ...
            case 'create' :
                ...
                if (is_wp_error($result))
                    throw new Exception(__('Error when trying to add new job category!', ET_DOMAIN), 400);
    ...
    try {
        ...
        switch ($method) {
            ...
            case 'create' :
                ...
                // if term can't be inserted throw an error
                if (is_wp_error($result))
                    throw new Exception(__('Error when trying to add new job type!', ET_DOMAIN), 400);
            ...
            case 'update':
                ...
                if (!is_wp_error($result)) {
                    ...
                    throw new Exception(__('Error when trying to update job type!', ET_DOMAIN));
                break;
...
function et_insert_expired_date($job_id) {
    ...
    if (in_array($template_id, array('rss', 'indeed', 'simplyhired', 'linked')))
        return;
...
function et_update_job($args = array()) {
    ...
    try {
        ...
        if (isset($args['method']) && $args['method'] == "edit_job") {
...
function et_create_jobs_response($post, $flush = false) {
    ...
    if ($post != NULL) {
        ...
        $company_zertifikat = get_field('company_certificate_pa', 'user_' . $post->post_author);
        //var_dump($company_zertifikat);
        if (!!$company_zertifikat)
            $zertifikat = get_stylesheet_directory_uri() . '/img/icon-pa-zertifikat.png';
        $company_label = get_field('company_label_verband', 'user_' . $post->post_author);
        if (!!$company_label)
            $company_label_verband = $company_label['url'];
        $clinici = '';
        $value = get_post_meta($post->ID, 'cfield-984', true);
        ...
        //if($flush || !$job ) {
        //get job package type
        $info_job = getJobPackageType(get_post_meta($post->ID, 'et_job_package', true));
        ...
        $job = array(
            ...
            'date_de' => formatDateDE($post->post_date),
            ...
            'clinici' => $clinici,
            'pack' => $info_job['pack'],
            'datasort' => $info_job['datasort']
        );
        if (isset($zertifikat))
            $job['company_zertifikat'] = $zertifikat;
        if (isset($company_label_verband))
            $job['company_label'] = $company_label_verband;
 ...
 *
 * @since 1.0
 * @return WP_Query object cfield-984
 */
function et_query_jobs($args) {
    ...
    $search_province = isset($_REQUEST['search_province']) && (!empty($_REQUEST['search_province']) ) ? $_REQUEST['search_province'] : '';
    if (!$search_province && isset($_SESSION['search_province'])) {
        $search_province = $_SESSION['search_province'];
    } else if ($search_province) {
        $_SESSION['search_province'] = $search_province;
    }
    if ($search_province != '') {
        $meta_query[] = array(
    ...
    if (isset($args['search_hospitals']) && isset($args['search_ambulances'])) {
        
    } else if (isset($args['search_hospitals']))
        $meta_query[] = array(
            'key' => 'cfield-984',
            'value' => 985
        );
    else if (isset($args['search_ambulances']))
        $meta_query[] = array(
            'key' => 'cfield-984',
            'value' => 986
        );

    resetRadiusSearch($args);
...
function et_get_post_job_url($url, $pagename = '', $params = array()) {
    ...
    if (($pagename == 'page-post-a-job' || $pagename == 'post-a-job') AND !empty($params['job_id'])) {
        $url = strtok($url, '?');
        $url .= $params['job_id'];
    }

    if (($pagename == 'page-process-payment' || $pagename == 'process-payment') AND !empty($params['paymentType'])) {
        $url = strtok($url, '?');
...
function et_get_job_status_labels($statuses = array('publish')) {
    ...
    foreach ($statuses as $value) {
        if ($value == 'publish')
            return __('LATEST JOBS', ET_DOMAIN);
