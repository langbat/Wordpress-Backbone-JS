...
abstract class ET_Payment {
    ...
    public static function get_support_payment_gateway() {
        ...
        $support = array(
            ...
            'debit' => array('label' => __('Lastschrift', ET_DOMAIN), 'active' => -1,
                'description' => __("Send your cash payment to our bank account", ET_DOMAIN)
            )
        );
...
class ET_Paypal extends ET_Payment {
    ...
    function __construct() {
        ...
        if ($this->_test_mod) { // in test mode : enpoint url is sandbox
            $this->_paypal_url = 'https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_cart&rm=2&business=';
        } else { // in realtime
            $this->_paypal_url = 'https://www.paypal.com/cgi-bin/webscr?cmd=_cart&rm=2&business=';
        }
    ...
    static function set_api($api = array()) {
        ...
        if (!self::is_enable()) {
            $gateways = self::get_gateways();
            if (isset($gateway['paypal']['active']) && $gateways['paypal']['active'] != -1) {
                ET_Payment::disable_gateway('paypal');
                return __('This payment gateway was disabled because API setting is invalid!', ET_DOMAIN);
            }
        }
        return true;
    }
...
function et_get_price_format($amount, $style = '') {
    ...
    if ($currency->align == "left")
        ...
        $format = '%2$s' . $format;
    return sprintf($format, number_format($amount, 2), $currency->icon);
}
