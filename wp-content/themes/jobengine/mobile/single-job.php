...
?>
	<div data-role="content" class="post-content resume-contentpage">
		...
		<div class="content-info content-text">
			<?php the_content(); ?>
				<?php do_action( 'je_single_job_fields', $job);  ?>
		</div>

		<?php if (et_get_auto_email('remind')) {?>
		<div class="content-info content-info-last content-text">
			<a href="#modal_apply" class="ui-btn-s btn-blue btn-wide modal-open"><?php  _e('Save this job',ET_DOMAIN); ?></a>
		</div>
		<?php } ?>
		<!-- <div class="content-tabs tabs-two">
			<ul class="tabs">
				<li class="activated"><a href="#share"><?php _e('Share',ET_DOMAIN); ?></a></li>
			</ul>
					...
					<ul class="listview">
						<li class="list-item">
							<a href="http://twitter.com/home?status=<?php the_title(); ?> - <?php the_permalink(); ?>" target="_blank">
								<span class="icon-twitter icon-social"><?php _e('Tweet this job',ET_DOMAIN); ?></span>
							</a>
						</li>
						<li class="list-item">
							<a href="http://www.facebook.com/sharer.php?u=<?php the_permalink();?>&t=<?php the_title(); ?>" target="blank">
								<span class="icon-facebook icon-social"><?php _e('Share on Facebook',ET_DOMAIN); ?></span>
							</a>
						</li>
						<li class="list-item">
							<a href="mailto:type email address here?subject=share this post from <?php echo bloginfo('name'); ?>&body=<?php the_title(); ?>&#32;&#32;<?php the_permalink(); ?>" title="Email to a friend" target="_blank">
								<span class="icon" data-icon="M" ><?php _e('Send via Email',ET_DOMAIN); ?></span>
							</a>
						</li>
					</ul>
				</div>
		...
		<div id="modal_apply" class="modal remind-popup" style="display: none">
