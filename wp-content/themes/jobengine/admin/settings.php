...
class ET_MenuSettings extends JE_AdminSubMenu {
    ...
    function __construct() {
        parent::__construct(__('Settings', ET_DOMAIN), __('SETTINGS', ET_DOMAIN), __('Manage how your job board looks and feels.', ET_DOMAIN), 'et-setting', 'icon-gear', 10);
    ...
    static function refresh_payment_plans() {
        ...
        foreach ($plans as $plan) {
            ...
            $new_plan->tax = get_post_meta($plan->ID, $prefix . 'price', true) * 0.19;
...
function et_refresh_payment_plans() {
    ...
    foreach ($plans as $plan) {
        ...
        $new_plan->price = get_post_meta($plan->ID, $prefix . 'price', true);
        $new_plan->tax = get_post_meta($plan->ID, $prefix . 'price', true) * 0.19;
        //$new_plan->discount_rate = get_post_meta($plan->ID, $prefix . 'price', true) * 0.19;
        $new_plan->duration = get_post_meta($plan->ID, $prefix . 'duration', true);
        $new_plan->featured = get_post_meta($plan->ID, $prefix . 'featured', true);
        $new_plan->quantity = get_post_meta($plan->ID, $prefix . 'quantity', true);
...
function et_refresh_payment_plans_amount($amount) {
    global $et_global;
    $prefix = $et_global['db_prefix'];
    ...
    foreach ($plans as $plan) {
        ...
        $new_plan->title = $plan->post_title;
        $new_plan->description = $plan->post_content;
        $new_plan->price = $amount;
        $new_plan->tax = $amount * 0.19;
        //$new_plan->discount_rate = get_post_meta($plan->ID, $prefix . 'price', true) * 0.19;
        $new_plan->duration = get_post_meta($plan->ID, $prefix . 'duration', true);
        $new_plan->featured = get_post_meta($plan->ID, $prefix . 'featured', true);
        $new_plan->quantity = get_post_meta($plan->ID, $prefix . 'quantity', true);
...
function et_payment_plans_zertifikat($order_data) {
    global $et_global;

    $cache = array();

    $new_plan = new stdClass;
    $new_plan->ID = $order_data['payer'];
    $new_plan->title = $order_data['title'];
    $new_plan->description = '';
    $new_plan->price = $order_data['total'];
    $new_plan->tax = $order_data['total'] * 0.19;
    $new_plan->duration = 1;
    $new_plan->featured = 1;
    $new_plan->quantity = 1;

    $cache = (array) $new_plan;
...
function et_get_payment_plans_amount($amount) {

    return et_refresh_payment_plans_amount($amount);
}

function et_get_payment_zertifikat($order_data) {

    return et_payment_plans_zertifikat($order_data);
}
        ...
        switch ($method) {
            ...
            case 'delete':
                ...
                if (is_wp_error($result))
                    throw new Exception(__("Can't delete payment plan", ET_DOMAIN));
    ...
    switch ($gateway) {
        ...
        default :
            ...
            if (!$return) {
                ...
                $response['msg'] = __('Payment API is invalid', ET_DOMAIN);
            }
            break;
    }
...
function et_get_auto_email($type = '') {
    $options = get_option('et_auto_email', array('apply' => 1, 'approve' => 1, 'remind' => 1, 'archive' => 1, 'reject' => 1, 'cash_notice' => 1, 'invoice_notice' => 1, 'mail_admin' => 1));
    if (!isset($options['cash_notice']) || !isset($options['invoice_notice']) || !isset($options['mail_admin'])) {
        $options['cash_notice'] = 1;
        $options['invoice_notice'] = 1;
        $options['mail_admin'] = 1;
    }
    return $options[$type];
}
...
function data_icon($data, $type = 'text') {
    if ($data == '')
        ...
        if ($type == 'link') {
            ...
            if ($validator->validate('link', $data))
                echo 3;
            else
                echo '!';
        }
        if ($type == 'email') {
            $validator = new ET_Validator();
            if ($validator->validate('email', $data))
                echo 3;
            else
                echo '!';
        }
