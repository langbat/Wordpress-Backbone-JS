...
class JE_Job_Edit extends ET_Base {
    ...
    public function job_meta_boxes($post) {
        ...
        <p>
            <input type="radio" name="et_apply_method" value="ishowtoapply" <?php if (isset($job['apply_method']) && $job['apply_method'] == 'ishowtoapply') echo 'checked="checked"' ?>> <?php _e('By description below', ET_DOMAIN) ?><br>
    ...
    public function save_job($post_id) {
        // verify nonce
        if (!isset($_POST['_et_nonce']) || !wp_verify_nonce($_POST['_et_nonce'], self::NONCE))
            return;
        ...
        //
        //Set featured by job_package
        $premium = array(20, 21, 22);
        $top = array(13, 15, 16, 17, 18, 19);
        if (in_array($_POST['et_job_package'], $premium))
            $this->update_job_meta($post_id, 'et_featured', '3');
        elseif (in_array($_POST['et_job_package'], $top))
            $this->update_job_meta($post_id, 'et_featured', '2');
        ...
        if (empty($_POST['et_expired_date'])) {//$this->update_job_meta($post_id, 'et_expired_date', date('Y-m-d h:i:s', strtotime("+30 days"))); // save expired date
        }
        else
